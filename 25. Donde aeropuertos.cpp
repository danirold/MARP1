...
ConjuntosDisjuntos ARM_Kruskal(GrafoValorado<int> const& G, int& coste_total, vector<Arista<int> >& ARM) {
    PriorityQueue<Arista<int> > pq(G.aristas());
    ConjuntosDisjuntos cjuntos(G.V());
    while (!pq.empty()) {
        auto a = pq.top();
        pq.pop();
        int v = a.uno(); int w = a.otro(v);
        if (!cjuntos.unidos(v, w)) {
            cjuntos.unir(v, w);
            ARM.push_back(a);
            coste_total += a.valor();
...
bool resuelveCaso() {
    int local, camin, coste;
    cin >> local >> camin >> coste;
    if (!cin) return false;

    GrafoValorado<int> G(local);
    for (int i = 0; i < camin; ++i) {
        int x, y, c;
        cin >> x >> y >> c;
    ...
    int coste_total = 0;
    int aero = 0;
    ConjuntosDisjuntos cjuntos = ARM_Kruskal(G, coste_total, ARM);

    aero += cjuntos.num_cjtos();
    cout << coste_total + aero * coste << ' ' << aero << '\n';
