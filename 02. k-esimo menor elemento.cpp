...
protected:
    ...
    struct TreeNode {
        ...
        TreeNode(T const& e, Link i = nullptr, Link d = nullptr,
            int alt = 1, int tamano_i = 1) : elem(e), iz(i), dr(d), altura(alt), tam_i(tamano_i) {}
    };
...
protected:
    ...
    T const& kesimo_elem(int k, Link raiz) {
        if (raiz == nullptr) {
            throw std::domain_error("??");
        }
        else {
            if (k == raiz->tam_i) return raiz->elem;
            else if (k < raiz->tam_i) return kesimo_elem(k, raiz->iz);
            else return kesimo_elem(k - raiz->tam_i, raiz->dr);
        }
    }
...
bool resuelveCaso() {
    int num_valores; cin >> num_valores;
