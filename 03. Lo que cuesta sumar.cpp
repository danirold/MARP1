...
template <typename T>
long long int resolver(PriorityQueue<T>& monticulo) {
    if (monticulo.size() == 1) {
        return 0;
    }
    else {
        ...
        while (monticulo.size() > 1) {
            long long int top = monticulo.top(); monticulo.pop();
            long long int sig_top = monticulo.top(); monticulo.pop();
            suma += top + sig_top;
            monticulo.push(top + sig_top);
        }
        return suma;
    }
}
...
bool resuelveCaso() {
    ...
    for (long long int i = 0; i < tam; ++i) cin >> v[i];

    PriorityQueue<long long int> monticulo(v);
