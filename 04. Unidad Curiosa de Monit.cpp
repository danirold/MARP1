...
bool operator<(tInfo const& elem, tInfo const& elem2) {
    return elem.cuando_toca < elem2.cuando_toca || (elem.cuando_toca == elem2.cuando_toca && elem.ident < elem2.ident);
}

bool resuelveCaso() {
    int num_usuarios; cin >> num_usuarios;

    if (num_usuarios == 0)
        return false;

    vector<tInfo> usuarios(num_usuarios);
    for (int i = 0; i < num_usuarios; ++i) {
        cin >> usuarios[i].ident >> usuarios[i].intervalo;
        usuarios[i].cuando_toca = usuarios[i].intervalo;
    }

    int num_envios; cin >> num_envios;

    PriorityQueue<tInfo> monticulo(usuarios);

    while (num_envios > 0) {
        tInfo top = monticulo.top();
        monticulo.pop();
        cout << top.ident << '\n';
        monticulo.push({ top.ident, top.cuando_toca + top.intervalo, top.intervalo });
        num_envios--;
    }
