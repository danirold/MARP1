...
bool operator <(tTema const& tema1, tTema const& tema2) {
    return tema1.info.citas > tema2.info.citas || (tema1.info.citas == tema2.info.citas && tema1.info.indice > tema2.info.indice);
}
...
bool resuelveCaso() {
    ...
    unordered_map<string, tInfo> mapa;
    IndexPQ<tTema> cola_prioridad(eventos);
    ...
    for (int i = 0; i < eventos; ++i) {
        ...
        if (c == "C") {
            ...
            if (!mapa.count(nombre)) {
                mapa.insert({ nombre, {citas, i, i} });
                cola_prioridad.push(i, {nombre, {citas, i, i}});
            }
            else {
                tInfo& info = mapa[nombre];
                info.citas += citas;
                info.indice = i;
                cola_prioridad.update(info.posicionEntrada, { nombre, info });
            }
        }
        else if (c == "E") {
            string nombre; cin >> nombre;
            ...
            queue<tTema> mejores;
            for (int i = 0; i < 3 && !cola_prioridad.empty(); ++i) {
                cout << i + 1 << ' ' << cola_prioridad.top().prioridad.nombre << '\n';
                mejores.push(cola_prioridad.top().prioridad);
                cola_prioridad.pop();
            }

            while (!mejores.empty()) {
                cola_prioridad.push(mejores.front().info.posicionEntrada, mejores.front());
                mejores.pop();
            }
        }
    }
