...
bool operator< (tPartido const& partido1, tPartido const& partido2) {
    return partido1.num_votos/(1 + partido1.num_escanos) > partido2.num_votos / (1 + partido2.num_escanos) ||
           (partido1.num_votos / (1 + partido1.num_escanos) == partido2.num_votos / (1 + partido2.num_escanos) && partido1.num_votos > partido2.num_votos) ||
           (partido1.num_votos / (1 + partido1.num_escanos) == partido2.num_votos / (1 + partido2.num_escanos) && partido1.num_votos == partido2.num_votos && partido1.prioridad < partido2.prioridad);
}

bool resuelveCaso() {
    double cand_present, esc_repart;
    cin >> cand_present >> esc_repart;

    if (cand_present == 0 && esc_repart == 0)
        return false;

    vector<double> sol(cand_present, 0);

    PriorityQueue<tPartido> partidos;

    double votos; double prioridad = 0;
    for (double i = 0; i < cand_present; ++i) {
        cin >> votos;
        partidos.push({ i, votos, 0, prioridad });
        prioridad++;
    }

    while(esc_repart > 0) {
        tPartido p = partidos.top();
        sol[p.numero]++;
        p.num_escanos++;
        partidos.pop();
