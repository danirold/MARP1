...
void relajar(Arista<int> const& a, vector<int>& dist, IndexPQ<int>& pq, int v) {
    int w = a.otro(v);
...
void Dijkstra(GrafoValorado<int> const& g, int origen, vector<int>& dist, IndexPQ<int>& pq) {
    dist[origen] = 0; 
    pq.push(origen, 0);
    while (!pq.empty()) {
...
bool resuelveCaso() {
    ...
    for (int i = 0; i < num_consultas; ++i) {
        ...
        Dijkstra(G, origen - 1, dist, pq);
        if (dist[final - 1] == INF) cout << "NO LLEGA\n";
        else cout << dist[final - 1] << '\n';
    }
