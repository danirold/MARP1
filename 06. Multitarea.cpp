...
bool operator< (tTarea const& tarea1, tTarea const& tarea2) {
    return (tarea1.comienzo < tarea2.comienzo || (tarea1.comienzo == tarea2.comienzo && tarea1.final < tarea2.final));
}


bool resuelveCaso() {
    int num_tar_unicas; cin >> num_tar_unicas;
    int num_tar_periodicas; cin >> num_tar_periodicas;
    long long int intervalo; cin >> intervalo;
    ...
    long long int com_unic, fin_unic;
    for (int i = 0; i < num_tar_unicas; ++i) {
        cin >> com_unic >> fin_unic;
        tareas.push({com_unic, fin_unic, 0, 'u'});
    }

    long long int com_per, fin_per, interv_per;
    for (int i = 0; i < num_tar_periodicas; ++i) {
        cin >> com_per >> fin_per >> interv_per;
        tareas.push({ com_per, fin_per, interv_per, 'p' });
    }

    bool sol = true;

    tTarea top;
    while (sol && !tareas.empty() && tareas.top().final < intervalo) {
        top = tareas.top();
        tareas.pop();
        if (!tareas.empty()) {
            if (top.final > tareas.top().comienzo) sol = false;
        }
        if (top.tipo == 'p') {
            tareas.push({ top.comienzo + top.periodo, top.final + top.periodo, top.periodo, 'p' });
        }
    }

    if (!tareas.empty()) {
        top = tareas.top(); tareas.pop();
        if (!tareas.empty() && tareas.top().comienzo < top.final && tareas.top().comienzo < intervalo) {
            sol = false;
        }
    }
