...
void relajar(Arista<int> const& a, vector<int>& dist, IndexPQ<int>& pq, int v, vector<int>& ulti) {
    int w = a.otro(v);
...
void Dijkstra(GrafoValorado<int> const& g, int origen, vector<int>& dist, IndexPQ<int>& pq, vector<int> &ulti) {
    dist[origen] = 0;
    pq.push(origen, 0);
    while (!pq.empty()) {
        int v = pq.top().elem; pq.pop();
        for (auto a : g.ady(v)) {
            relajar(a, dist, pq, v, ulti);
        }
    }
}
...
bool resuelveCaso() {
    ...
    for (int i = 0; i < num_consultas; ++i) {
        ...
        Dijkstra(G, origen - 1, dist, pq, ulti);
        if (dist[final - 1] == INF) cout << "NO LLEGA\n";
        else {
            cout << dist[final - 1] << ": ";
            int i = final - 1;
            deque<int> camino;
