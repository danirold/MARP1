...
void relajar(Arista<int> const& a, vector<int>& dist, IndexPQ<int>& pq, int v, vector<Arista<int>> & ulti, vector<int> & num_aristas) {
    int w = a.otro(v);
    if (dist[w] > dist[v] + a.valor()) {
        dist[w] = dist[v] + a.valor();
        ulti[w] = a;
        num_aristas[w] = num_aristas[v] + 1;
        pq.update(w, dist[w]);
    }
    else if (dist[w] == dist[v] + a.valor()) {
        if (num_aristas[w] > num_aristas[v] + 1) {
            num_aristas[w] = num_aristas[v] + 1;
            ulti[w] = a;
        }
    }
...
void Dijkstra(GrafoValorado<int> const& g, int origen, vector<int> & num_aristas, vector<int>& dist, IndexPQ<int>& pq, vector<Arista<int>> & ulti) {
    dist[origen] = 0;
    num_aristas[origen] = 0;
    pq.push(origen, 0);
    while (!pq.empty()) {
        int v = pq.top().elem; pq.pop();
        for (auto a : g.ady(v)) {
            relajar(a, dist, pq, v, ulti, num_aristas);
        }
    }
}
...
bool resuelveCaso() {
    ...
    cin >> num_vertices >> num_aristas;;
    if (!cin) return false;
    GrafoValorado<int> G(num_vertices);
    GrafoValorado<int> G_camino(num_vertices);
    for (int i = 0; i < num_aristas; ++i) {
        int vertice1, vertice2, valor;
        cin >> vertice1 >> vertice2 >> valor;
        G.ponArista({ vertice1 - 1, vertice2 - 1, valor });
        G_camino.ponArista({ vertice1 - 1, vertice2 - 1, 1 });
    }
    ...
    for (int i = 0; i < num_consultas; ++i) {
        ...
        vector<int> num_aristas(G.V(), INF);
        vector<Arista<int> > ulti(G.V());
        IndexPQ<int> pq(G.V());
        int origen, final; cin >> origen >> final;
        Dijkstra(G, origen - 1, num_aristas, dist, pq, ulti);

        vector<int> dist_cam(G.V(), INF);
        vector<int> num_aristas_cam(G.V(), INF);
        vector<Arista<int> > ulti_cam(G.V());
        IndexPQ<int> pq_cam(G.V());
        Dijkstra(G_camino, origen - 1, num_aristas_cam, dist_cam, pq_cam, ulti_cam);

        if (dist[final - 1] == INF) cout << "SIN CAMINO\n";
        else {
            cout << dist[final - 1] << ' ';
            if (num_aristas[final - 1] == num_aristas_cam[final - 1]) cout << "SI\n";
            else cout << "NO\n";
        }
    }
