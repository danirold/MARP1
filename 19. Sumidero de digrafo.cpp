...
void grados(Digrafo const& Dg, vector<tInfo> & sol) {
    for (int i = 0; i < sol.size(); ++i) {
        sol[i].grado_salida = Dg.ady(i).size();
        for (int v : Dg.ady(i)) {
            sol[v].grado_entrada++;
        }
    }
...
bool resuelveCaso() {
    ...
    vector<tInfo> sol(Dg.V());
    for (int i = 0; i < Dg.V(); ++i) {
        sol[i].grado_entrada = 0; 
        sol[i].grado_salida = 0;
    }
    ...
    int vertice;
    for (int i = 0; i < sol.size() && !haySumidero; ++i) {
        if (sol[i].grado_entrada == Dg.V() - 1 && sol[i].grado_salida == 0) {
            haySumidero = true;
            vertice = i;
        }
    }
