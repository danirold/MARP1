...
void relajar(Arista<int> const& a, vector<int>& dist, IndexPQ<int>& pq, int v, vector<int> & cand) {
    int w = a.otro(v);
...
void Dijkstra(GrafoValorado<int> const& g, int origen, vector<int>& dist, IndexPQ<int>& pq, vector<int> & cand) {
    dist[origen] = 0;
    cand[0] = 1;
    pq.push(origen, 0);
    while (!pq.empty()) {
...
bool resuelveCaso() {
    ...
    IndexPQ<int> pq(G.V());
    vector<int> cand(G.V(), 0);

    Dijkstra(G, 0, dist, pq, cand);
    cout << cand[G.V() - 1] << '\n';
