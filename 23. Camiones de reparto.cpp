...
void dfs(GrafoValorado<int> const& g, int v, int umbral, vector<bool> &visit, queue<int> & valores) {
    visit[v] = true;
    for (auto a : g.ady(v)) {
        if (a.valor() >= umbral) {
            int w = a.otro(v);
...
bool resuelveCaso() {
    ...
    for (int i = 0; i < consultas; ++i) {
        int vertice1, vertice2, arista;
        cin >> vertice1 >> vertice2 >> arista;

        vector<bool> visit(G.V(), false);
        queue<int> valores;

        dfs(G, vertice1 - 1, arista, visit, valores);

        bool sol = false;
        while (!valores.empty() && !sol) {
            if (valores.front() == vertice2) {
                sol = true;
            }
            valores.pop();
        }
